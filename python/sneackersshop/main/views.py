...
def search(request):
    ...
    product = Sneakercard.objects.filter(title__contains=searched_sneakers)
...
def cart(request):
    ...
    product_cart = Sneakercard.objects.filter(id__in=cart_session)
    all_products_sum = 0
    ...
    if request.method == 'POST':
        if len(cart_session) == 0:
            ...
            for i in range(len(cart_session)):
                ...
                for i in set_list:
                    product = Sneakercard.objects.get(id = 1)
